...
'''sha256代码来自于https://github.com/keanemind/Python-SHA-256，仅对长度扩展攻击进行测试'''
...
if __name__ == '__main__':
    ...
    #模拟进行长度扩展攻击
    M2 = 'LENGTH EXTENSION ATTACK'
    _M1 = sha256_padding(M1, len(key))
    Forge_message, h2 = length_extansion_attack(M1, h, len(key), M2)
    print('伪造 :', Forge_message)

    #验证是否攻击成功
    _M2 = bytearray(M2, 'ascii')
    v = sha256(key_hash + _M1 + _M2).hexdigest()
    assert (h2 == v)
    print(h2)
